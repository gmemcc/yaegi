...
func (conf *Config) Check(path string, fset *token.FileSet, files []*ast.File, info *Info, depth int) (*Package, error) {
	pkg := NewPackage(path, "")
	return pkg, NewChecker(conf, fset, pkg, info).Files(files, depth)
}
